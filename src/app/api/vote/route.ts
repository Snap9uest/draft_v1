...
export async function POST(request: Request) {
  try {
    ...
    if (!roomCode || !sessionToken || !photoId) {
      return fail("투표 정보가 빠졌어요. 화면을 새로고침한 뒤 다시 눌러 주세요.");
    }
    ...
    const { data: voter } = await db
      ...
      .maybeSingle();
    if (!voter) return fail("먼저 파티에 입장해 주세요.", 403);
    ...
    const { data: photo } = await db
      ...
      .eq("hidden", false)
      .maybeSingle();
    if (!photo) return fail("지금은 투표할 수 없는 사진이에요. 다른 사진을 골라 주세요.", 404);
    ...
    if (error) return fail("투표를 저장하지 못했어요. 다시 눌러 주세요.", 500);

    return NextResponse.json({ ok: true });
  } catch (error) {
    return fail(error instanceof Error ? error.message : "투표를 저장하지 못했어요. 다시 눌러 주세요.", 500);
  }
}
